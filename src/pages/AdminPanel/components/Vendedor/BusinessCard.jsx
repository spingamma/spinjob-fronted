    ...
    <div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100 flex flex-col h-full hover:shadow-md transition-shadow group">
      <div className="flex items-center gap-3 mb-4">
        <div className="w-12 h-12 rounded-xl bg-gray-50 border border-gray-100 overflow-hidden shrink-0">
          <img 
            src={b.image || `https://ui-avatars.com/api/?name=${encodeURIComponent(b.name)}&background=F8F9FA&color=1A535C`} 
            alt={b.name} 
            className="w-full h-full object-cover"
          />
        </div>
        <div className="flex-1 min-w-0">
          <h3 className="font-bold text-primary truncate" title={b.name}>{b.name}</h3>
      ...
      <div className="mt-auto pt-4 border-t border-gray-50">
          ...
          {b.owner_id && !b.is_held_by_seller ? (
            <div className="flex items-center gap-2 text-primary bg-primary/10 p-3 rounded-xl border border-primary/20">
              ...
              <div>
                ...
                <p className="text-[10px] text-primary/80">Transferido exitosamente.</p>
              </div>
            </div>
          ) : b.is_held_by_seller ? (
          ...
          {b.is_held_by_seller && b.possible_owners && b.possible_owners.length > 0 && (
            <div className="flex flex-col gap-3 mt-1">
              {b.possible_owners.map((po, idx) => (
                <div key={idx} className="flex flex-col gap-2 p-3 rounded-xl border border-secondary/20 bg-secondary/5 relative">
                  <div className="flex items-start gap-2 text-secondary">
                    <AlertCircle size={18} className="shrink-0 mt-0.5" />
                    <div className="flex-1 min-w-0">
                      <p className="text-xs font-bold truncate">Posible Dueño: {po.name}</p>
                      <p className="text-[10px] text-secondary/80">{po.phone}</p>
                    </div>
                  </div>
                  <button 
                    onClick={() => handleTransfer(b, po)}
                    disabled={transfering === b.slug}
                    className="w-full bg-primary hover:bg-primary/90 text-white py-1.5 rounded-lg text-[11px] font-bold flex items-center justify-center gap-2 transition-colors disabled:opacity-50"
                  >
        ...
        {(!b.owner_id || b.is_held_by_seller) && (
          <button
            ...
            className="w-full bg-gray-100 hover:bg-gray-200 text-gray-500 py-2 rounded-xl text-xs font-bold flex items-center justify-center gap-2 transition-colors mt-2"
          >
