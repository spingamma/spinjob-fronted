...
function Perfil() {
  ...
  useEffect(() => {
    ...
    // Función asíncrona con sistema de reintentos para soportar "Cold Starts" de Render
    const obtenerPerfil = async (intentos = 0) => {
      try {
        // Usamos el endpoint específico en lugar de traer todo el directorio
        ...
        if (res.ok) {
          ...
          // 404 No encontrado o 403 Expirado (Validaciones directas desde main.py)
          if (isMounted) {
            setProfesional(null);
            setCargando(false);
          }
        } else {
          // Posible 502/503 temporal si Render está despertando la base de datos o el dyno
        ...
        if (intentos < 10 && isMounted) {
          // Mejorar la UX cambiando el mensaje tras demoras
          ...
          // Reintento recursivo con un retraso de 3.5 segundos (Soporta demoras de ~35 segs)
    ...
    return () => {
      isMounted = false; // Cleanup para evitar fugas de memoria si el usuario sale rápido
    };
  }, [slug, API_URL]);
