...
## 11. Presencia y Preservación de Identificadores Primarios (Anti-Ghosting de IDs)
🚨 **CRÍTICO:** Las llaves primarias (`id`, `uuid` o equivalente) son el pilar de la lógica relacional y de estado. Omitirlas o perderlas en el tránsito de red causa fallos de permisos difíciles de rastrear (como `isOwner` evaluando a `false`).
- **Backend (Emisión):** Al desarrollar endpoints que retornen datos de una entidad, especialmente **Autenticación (Login/Registro)** o recuperación de sesiones (`/me`), **ESTÁS OBLIGADO** a incluir el identificador primario en el payload de respuesta/DTO.
- **Frontend (Recepción):** Está **ESTRICTAMENTE PROHIBIDO** desestructurar o filtrar manualmente los payloads del servidor descartando propiedades estructurales clave. Si recibes un objeto de sesión (ej. en un `login`), debes guardar íntegramente el `id` en el almacenamiento local o estado. No asumas que la UI "solo necesita datos visuales" como nombre o email.
