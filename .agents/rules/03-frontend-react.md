...
8b. **Verificación Estricta de Imports y Rutas Relativas:** Antes de utilizar variables globales o de configuración (ej. URLs de API o tokens), verifica OBLIGATORIAMENTE que el import exista y computa mentalmente la ruta relativa exacta considerando la profundidad actual del archivo.
...
17b. **Paridad Estricta de Payload (Frontend-Backend):** Antes de estructurar el body de una petición (POST/PUT), DEBES OBLIGATORIAMENTE leer el esquema de validación esperado en el backend (ej. Pydantic o Swagger) para asegurar que se envíen todos los campos requeridos (evitando HTTP 422) y estructurarlos tal como se declaran en la API.
...
20. **Programación Defensiva Obligatoria en Colecciones (Anti-Crash):** Siempre que se itere (`.map`, `.filter`, `.reduce`) sobre estructuras anidadas (arrays) de origen dinámico (API o hooks), **ESTÁ ESTRICTAMENTE PROHIBIDO** asumir que la colección existe. Se debe utilizar siempre un fallback a un arreglo vacío: `(objeto.coleccion || []).map(...)` o optional chaining, previniendo excepciones fatales de *TypeError: Cannot read properties of undefined* que rompen el árbol de React.
21. **Simetría Estricta de Estado Compartido (URL Params / Props):** Al leer o escribir estados que conectan diferentes capas del frontend (ej. un hook que actualiza la URL y una vista que lee esa URL), se **DEBE GARANTIZAR SIMETRÍA EXACTA** en los nombres de las claves. No se deben mezclar alias (ej. escribir `?buscar=` y leer `searchParams.get('search')`).

## Autorización y Control de Acceso (RBAC)
20. **Prevención de UI Cruzada (Cross-Role UI):** Queda estrictamente prohibido renderizar acciones críticas o sensibles (aprobaciones, pagos, cancelaciones, asignaciones) basándose **únicamente** en el estado de una entidad (ej. `estado === 'pendiente'`). **DEBES OBLIGATORIAMENTE** verificar de forma conjunta el rol, categoría o permisos del contexto actual (`estado === 'pendiente' && !isRolRestringido`) para evitar que actores no autorizados interactúen con flujos que no les corresponden.
